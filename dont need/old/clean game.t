...
var buildings : flexible array 1..0 of int
var location : array 1..2 of string
var place : area
var player : person
var inn : area
var count, endCount : int
...
proc innStart
    Sprite.ChangePic (place.sprite,place.pic(2))
    place.y := maxy - 100
    place.x := maxx div 2
    ...
    if hasch = true then
        ...
        if keyPress (key) = "menu" then
            ...
            if whatdotcolour (maxx div 2 - 23, maxy div 2 + 45) = 18 and whatdotcolour (maxx div 2+23, maxy div 2 + 45) = 18 then
                place.y -=10
            elsif whatdotcolour (maxx div 2 - 23, maxy div 2 + 45) = 11 and whatdotcolour (maxx div 2+23, maxy div 2 + 45) = 11 then
                location (2) := "inn"
                cls
                innStart
            else
...
proc gameInitialize
    place.pic(2) := Pic.FileNew ("images/hotel ground floor.bmp")
    place.pic(1) := Pic.FileNew ("images/town 1.bmp")
    place.sprite := Sprite.New (place.pic(1))
    place.x := maxx div 2
    place.y := maxy div 2
    ...
    new buildings, 3
    drawfill (1,1,18,18)
    buildings (1) := Pic.FileNew ("images/house.bmp")
    buildings (2) := Pic.FileNew ("images/inn.bmp")
    %buildings (3) = Pic.FileNew ("images/
    location (1) := "town1"
    location (2) := "town"
end gameInitialize
...
loop
    move
    if location (1) = "town1" then
        if location (2) = "town" then
            Pic.Draw (buildings (1), place.x-245,place.y+660, picCopy)
            Sprite.Show (place.sprite)
            Pic.Draw (buildings (2), place.x-975,place.y+104, picCopy)
            % elsif location (2) := "inn"
        end if
    end if
    delay (5)
    exit when done = true
